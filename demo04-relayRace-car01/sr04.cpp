...
static bool beep;
unsigned getDistance(void) { return distance / 1053; }  //distance in cm
...
void sr04(unsigned phase) {
  if (phase == 7) {
    ...
    TIM1->CCMR2 = PWM;
		if (beep) TIM1->CCER |= TIM_CCER_CC3NE;     //enable o/p pin
  } else {
    if (!phase) {
...
void sr04(bool b) { beep = b; }			// true to detect; false to keep quiet
